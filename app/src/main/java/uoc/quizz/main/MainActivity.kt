...
class MainActivity : AppCompatActivity() {
    ...
    private fun questionManager(){
        if (questionsInitialized) {
            ...
            questions = sqliteHelper.readAllQuestionsFromDb()
        }
    ...
    private fun initQuestionsToDb() {
        sqliteHelper.deleteQuestion(questions[0])
        sqliteHelper.deleteQuestion(questions[1])
        sqliteHelper.deleteQuestion(questions[2])
        insertQuestionToDb(questions[0])
        insertQuestionToDb(questions[1])
        insertQuestionToDb(questions[2])
        //Read SQlite TEST
        //var questionTest = sqliteHelper.readQuestionFromDb(questions[2].id)
        //var questionTest2 = sqliteHelper.readAllQuestions(questions[2].id)
    ...
    private fun bindingManager(binding: ActivityMainBinding) {
        ...
        binding.button.setOnClickListener { _ ->
            if(selectedOption != "") {
                ...
                sqliteHelper.deleteQuestion(questions[questionNum])
                insertQuestionToDb(questions[questionNum])
                openResultActivity()
            }
            else {
    ...
    private fun insertQuestionToDb(q: Question) {
        val status = sqliteHelper.insertQuestionToDb(q)
